...
pub async fn edit_async(_lua: Lua, args: Option<String>) -> LuaResult<()> {
    let path = args;
    ...
    let yaml_raw = std::fs::read_to_string(&pth).map_err(|e| mlua::Error::external(e))?;
    let yaml: serde_yaml::Value =
        serde_yaml::from_str(&yaml_raw).map_err(|e| mlua::Error::external(e))?;

    let obj: DynamicObject = serde_yaml::from_value(yaml).map_err(|e| mlua::Error::external(e))?;
    ...
    if let Some((ar, caps)) = discovery.resolve_gvk(&gvk) {
        ...
        return Err(mlua::Error::RuntimeError(
            "Cannot edit document for unknown".into(),
        ));
    }
