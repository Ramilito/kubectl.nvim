...
//!
//! Provides two session types:
//! - `Session` - Terminal-based rendering via CrosstermBackend (legacy)
//! - `BufferSession` - Native Neovim buffer rendering via NeovimBackend
...
use crate::ui::{
    events::{bytes_to_event, is_quit_event, parse_message, ParsedMessage},
...
// ============================================================================
// BufferSession - Native Neovim buffer rendering
// ============================================================================

/// UI session that renders to a native Neovim buffer instead of a terminal.
///
/// This provides the same functionality as Session but outputs structured
/// data (lines + extmarks) that can be applied to a regular Neovim buffer,
/// enabling native vim motions, search, yank, etc.
...
async fn run_buffer_ui(
    ...
    open: Arc<AtomicBool>,
    cols: Arc<AtomicU16>,
    _rows: Arc<AtomicU16>, // Not used - we use fixed large height for native scrolling
) {
    // Wait a moment for Lua to send the correct dimensions
    time::sleep(Duration::from_millis(50)).await;

    let initial_w = cols.load(Ordering::Acquire);
    // Use a large height for buffer-based rendering so all content fits
    // Neovim will handle scrolling naturally
    let initial_h = 500u16; // Large enough for most content
            ...
            msg = rx_in.recv() => {
                match msg {
                    Some(bytes) => {
                        ...
                        if needs_redraw {
                            // Check for width resize (height stays large for native scrolling)
                            let new_w = cols.load(Ordering::Acquire);
                            let current_size = term.backend().size().unwrap_or_default();

                            if new_w != current_size.width {
                                term.backend_mut().resize(new_w, 500);
                                let _ = term.resize(Rect::new(0, 0, new_w, 500));
                            }
